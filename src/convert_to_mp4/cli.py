...
def _check_and_upgrade() -> None:
    ...
    for cmd in upgrade_commands:
        try:
            ...
            logger.debug("Upgrade with %s failed", cmd[1], exc_info=True)
...
def _main(
    ...
    console.print(f"[dim]convert-to-mp4 v{version('convert-to-mp4')}[/dim]")
    _check_and_upgrade()
